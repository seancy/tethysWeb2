      ...
      <div class="contain_width">
        ...
        <template v-else>
          <div class="content-hred">
            <p class="h2"><strong>AGENT </strong>TO JOIN</p>
            <p class="h3">代理加盟</p>
          </div>
        </template>
      </div>
          ...
          <ul class="psn_menu" id="footer_menu">
            <template v-for="(item,key) in menu.list">
              <li v-if="item.code === code" @click="changeTab(item.code)"><a href="javascript:;" class="active"><span
                class="icon_sprite icon_about"></span>{{ item.title }}</a></li>
                        ...
                        <form id="apply_form">
                          <dl class="psn_info">
                            <dd>
                              <ul class="style_item">
                                <li>
                                  <span class="label">帐号<span class="color_red">※</span></span>
                                  <span class="form"><input type="text" class="formInput error" id="username"
                                                            name="username" placeholder="请输入4-15位英文与数字组合"></span>
                                  <span class="ui_error">用户名不能为空</span>
                                </li>
                                <li>
                                  <span class="label">登录密码<span class="color_red">※</span></span>
                                  <span class="form"><input type="text" class="formInput" id="password" name="password"
                                ...
                                <li>
                                  ...
                                  <span class="ui_error">必须与银行帐户名称相同，否则不能出款！</span>
                                </li>
                                <li>
                                  <span class="label">银行卡号<span class="color_red">※</span></span>
                                  <span class="form"><input type="text" class="formInput" id="bankCardNo"
                                                            name="bankCardNo" placeholder="请输入您的银行卡号"></span>
                                  <span class="ui_error"><span class="icon_sprite icon_ok"></span></span>
                                </li>
                                <li>
                                ...
                                <li>
                                  ...
                                  <span class="form"><input type="text" class="formInput" id="bankDeposit"
                                                            name="bankDeposit" placeholder="请输入开户行地址"></span>
                                  <span class="ui_error"><span class="icon_sprite icon_ok"></span></span>
                                </li>
                                <li>
                                  <span class="label">手机号<span class="color_red">※</span></span>
                                  <span class="form"><input type="text" class="formInput" id="telephone"
                                                            name="telephone"
                              ...
                              <div class="sing-up_msg_plus">
                                ...
                                <label for="agree">我已届满合法博彩年龄，且已阅读并同意</label><a href="javascript:;">代理注册协议</a>
                              </div>
                            </dd>
                          </dl>
                          ...
                          <div class="form_submit">
                            <a class="formBtn modalBtn" href="#modalSuccess">立即注册</a>
                            <!--注册成功讯息-->
        ...
        switch (id) {
          case 'username':
            ...
            if (val == '' || val == ' ' || val.length == 0) {
              ...
              common.ajax('member/username/check', {username: val}, function (data) {
                if (data && data.result) {
                  if (data.apistatus === 1) {
                    $(el).removeClass('backgroundErr').parent('div').next().html('<span class="icon_ok"></span>').removeClass('red');
                  }
                  else {
                    $(el).addClass("backgroundErr").parent('div').next().html('用户名重复').addClass("red");
                    _self.getCode();
                  }
                }
              }, 'get', function (data) {
                //  $(el).addClass("backgroundErr").parent('div').siblings("div").html(data.errorMsg).addClass("red");
              });
            }
            break;
          case 'password':
          ...
          case 'realname':
            ...
            if (val == '' || val == ' ' || val.length == 0) {
              ...
              $(el).addClass("backgroundErr").parent('div').next().html('必须与银行帐户名称相同，否则不能出款！');
            }
            else if (val.length < 2) {
              $(el).addClass("backgroundErr").parent('div').next().html('请至少输入2个字').addClass("red");
            }
            else if (val.length > 5) {
              $(el).addClass("backgroundErr").parent('div').next().html('请最多输入5个字').addClass("red");
            }
            else {
              $(el).removeClass('backgroundErr').parent('div').next().html('<span class="icon_ok"></span>').removeClass('red');
            }
          ...
          case 'bank':
            ...
            if (val == '' || val == ' ' || val.length == 0 && isRequired == 1) {
              $('#bankSelect').addClass("backgroundErr").parent('div').next().html('请选择银行').addClass("red");
            }
            else {
              $('#bankSelect').removeClass('backgroundErr').parent('div').next().html('<span class="icon_ok"></span>').removeClass('red');
            }
            break;
          case 'code':
            val = el && el[0].value || '';
            if (val == '' || val == ' ' || val.length == 0) {
              ...
              $(el).addClass('backgroundErr').parent().next().next().html('验证码错误').addClass('red').css("padding-left", "10px");
            }
            else {
              $(el).removeClass('backgroundErr').parent().next().next().html('').removeClass('red');
            }
